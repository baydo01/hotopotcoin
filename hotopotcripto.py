...
# --- SAYFA AYARLARI ---
st.set_page_config(page_title="Hedge Fund Manager: Dynamic V6", layout="wide", initial_sidebar_state="expanded")
...
def optimize_dynamic_weights(df, n_states, commission, alloc_capital, validation_days=15):
    df = df.copy()
    ...
    if len(df) < validation_days + 10: return 0.7, 0.3  # default
...
def run_multi_timeframe(df_raw, params, alloc_capital):
    ...
    for tf_name,tf_code in timeframes.items():
        ...
        for idx,row in df.iterrows():
            ...
            score_signal=1 if row['custom_score']>=3 else (-1 if row['custom_score']<=-3 else 0)
            decision=w_hmm*hmm_signal + w_score*score_signal
            target_pct=1.0 if decision>0.25 else (0.0 if decision<-0.25 else None)
            ...
            if idx==df.index[-1]:
                ...
                history={"Fiyat":price,"HMM":regime,"Puan":int(row['custom_score']),"Öneri":"AL" if decision>0.25 else ("SAT" if decision<-0.25 else "BEKLE"),
...
# --- ARAYÜZ ---
st.title("🏆 Hedge Fund Manager: Dynamic HMM+Score V6")
st.markdown("### 📊 Güncel + Geçmiş Veri Dengeli, Dinamik Ağırlık Optimizasyonlu Backtest")
...
with st.sidebar:
    ...
    st.info("Sistem geçmiş veriyi dikkate alır ve son 3 haftayı optimize ederek HMM/Puan ağırlığını belirler.")
